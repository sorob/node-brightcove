...
var buildUrl = function buildUrl(command, options) {
  var isHttpPost = this.commands.write.hasOwnProperty(command);
  var service = (this.commands.write.hasOwnProperty(command)) ? this.WriteService : this.ReadService,
    url = new Uri(service);

  if (!isHttpPost) {
    url.addQueryParam('command', command)
...
api.prototype.createPlaylist = function createPlaylist(playlist, callback) {
  ...
  this.makeRequest(this.commands.write.create_playlist, {
    playlist: playlist
  }, callback);
}
api.prototype.createVideo = function createVideo(videofile, options, callback) {
  var params = options || {};
  params.token = this.getToken();
  ...
  // params.file = fs.createReadStream(videofile);
  // params.JSONView = JSON.stringify(params);
  // params.jsonRequest1 = JSON.stringify(params);

  request.post({
    url: this.WriteService,
    formData: {
